...
export default async function DomainsPage({ searchParams }: { searchParams: Promise<Record<string, string | undefined>> }) {
  ...
  return <>
    ...
    <form className={`${styles.filterForm} ${styles.domainsFilterForm}`}>
      <div className={styles.filterPrimary}>
        <label className={styles.field}>
          <span className="sr-only">Buscar negocio o dominio</span>
          <input className={styles.input} name="q" defaultValue={query.q} placeholder="Buscar negocio o dominio" />
        </label>
        <button className="button button-secondary" type="submit">Buscar</button>
      </div>
      <ResponsiveFilters active={Boolean(query.pending || query.renewal)}>
        <summary>
          Más filtros{query.pending || query.renewal ? " · activos" : ""}
        </summary>
        <div className={styles.filterGrid}>
          <label className={styles.field}>
            <span><input type="checkbox" name="pending" value="1" defaultChecked={query.pending === "1"} /> Meses pendientes</span>
          </label>
          <label className={styles.field}>
            <span><input type="checkbox" name="renewal" value="1" defaultChecked={query.renewal === "1"} /> Renovaciones próximas</span>
          </label>
