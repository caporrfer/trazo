...
const demoSubmissions = new Map<string, string>();
...
function demoRateLimited(key: string) {
  const now = Date.now();
  const recent = (demoAttempts.get(key) || []).filter(
...
export async function POST(
  ...
  if (isDemoMode() || !hasSupabaseConfig()) {
    ...
    const existingId = demoSubmissions.get(submissionKey);
    if (existingId)
      return NextResponse.json(
    ...
    demoSubmissions.set(submissionKey, saved.id);
    return NextResponse.json(
